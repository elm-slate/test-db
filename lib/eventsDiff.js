...
const eventsValidationString = fs.readFileSync(path.resolve(__dirname, '../sql/loadPersonDataValidation.sql'), 'utf8');
...
const getEvents = co.wrap(function *(connectionUrl, selectStatement, maxEventsPerRead) {
	...
	try {
		...
		return yield dbEvents.getEventsFromStream(eventStream, maxEventsPerRead);
	}
	finally {
...
const eventsDiff = co.wrap(function *(events1ConnectionParams, events2ConnectionParams, logger, maxEventsPerRead, maxDiffs, connectTimeout, validateTables) {
	try {
		...
		const maxDbClient1EventId = yield dbEvents.getMaximumEventId(dbClient1.dbClient);
		const dbClient1RowCount = yield dbEvents.getEventCount(dbClient1.dbClient);
		...
		const maxDbClient2EventId = yield dbEvents.getMaximumEventId(dbClient2.dbClient);
		const dbClient2RowCount = yield dbEvents.getEventCount(dbClient2.dbClient);
...
(function() {
	program
		...
		.option('-v, --validate-tables', 'optional parameter.  if specified, validate "testingStats" in each events table.  must have created each event column jsonb object with "testingStats property".  see lib/loadPersonData.js.')
